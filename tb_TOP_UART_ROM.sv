...
module tb_TOP_UART_ROM;
    ...
    initial begin
        ...
        forever begin
            ...
            $display("[%0t] UART Byte[%0d] = %02h",
                     $time, captured_count, rx_byte);
    ...
    task automatic check_byte(
        ...
        begin
            if (captured_bytes[index] !== expected) begin
                $fatal(1,
                       "Byte %0d mismatch: expected=%02h actual=%02h",
                       index, expected, captured_bytes[index]);
            end
        end
    endtask
    ...
    initial begin
        ...
        // Initial IDLE status: 32'h0000_0000, LSB byte first.
        ...
        // 4-byte initial IDLE + 4-byte EXPORT + 8-byte pixels
        // + 4-byte return IDLE status.
        ...
        // EXPORT status = 32'h1000_0000, LSB byte first.
        check_byte(4,  8'h00);
        check_byte(5,  8'h00);
        check_byte(6,  8'h00);
        check_byte(7,  8'h10);

        // The first four sunset.mem words are 16'h53FB.
