...
def read_exact(stream: BinaryIO, size: int, show_progress: bool = False) -> bytes:
    """Read exactly size bytes, tolerating serial timeout returns."""
    received = bytearray()
    next_percent = 10
...
def parse_status(raw_status: bytes) -> tuple[int, int]:
    """Decode the four LSB-byte-first status bytes."""
...
def rgb444_payload_to_image(payload: bytes, width: int, height: int) -> Image.Image:
    """Convert [low 8 bits, upper 4 bits] RGB444 pixels to RGB888."""
    pixel_count = width * height
    expected_size = pixel_count * 2

    if len(payload) != expected_size:
        raise ValueError(
            f"Pixel 데이터 크기 오류: expected={expected_size}, actual={len(payload)}"
        )
    ...
    for index in range(pixel_count):
        ...
        # Expand each 4-bit channel to 8 bits: 0xA -> 0xAA.
...
def receive_from_uart(args: argparse.Namespace) -> None:
    try:
        ...
        raise SystemExit(
            "pyserial이 없습니다. 먼저 'pip install -r requirements.txt'를 실행하세요."
        ) from exc
    ...
    with serial.Serial(
        ...
        print("FPGA의 EXPORT 버튼을 누르세요. Ctrl+C를 누르면 종료됩니다.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="FPGA UART의 RGB444 Pixel 데이터를 받아 PNG로 저장합니다."
    )
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--port", help="Windows COM 포트 예: COM5")
    source.add_argument(
        "--input-bin",
        help="UART 없이 변환할 Pixel payload 파일(2Byte/Pixel)",
    )
    ...
    parser.add_argument(
        "--continuous",
        action="store_true",
        help="한 장 저장한 뒤 종료하지 않고 다음 EXPORT를 계속 기다립니다.",
    )
    parser.add_argument(
        "--keep-buffer",
        action="store_true",
        help="프로그램 시작 시 기존 UART 수신 버퍼를 비우지 않습니다.",
    )
    parser.add_argument(
        "--save-raw",
        action="store_true",
        help="PNG와 함께 수신 Pixel payload를 .bin으로 저장합니다.",
    )
